...
public interface FacturaCustomRepository extends JpaRepository<Factura, Long> {
    ...
    @Query("SELECT f.facturaId, f.fechaFactura, c.nombre, f.total, df.detalleId, p.nombre, df.cantidad, df.precioUnitario "
            +
            "FROM Factura f " +
            "JOIN f.cliente c " +
            "JOIN f.detalles df " +
    ...
    List<Object[]> obtenerFacturaConDetalles(@Param("facturaId") Long facturaId);
